...
$(document).on('ready',function(){
	...
	var imageSlides=[];var pageSections=[];
	var pageAnchors=[];var scrollOverflow=true;
	var css3=true;
	...
	windowVar.on('load',function(){
					...
					sendRequest:function(p){
						...
						$.ajax({
							...
							success:function(data){
									...
									console.log('Se han enviado los datos satisfactoriamente');
								
							},error:function(jqXHR,textStatus,errorThrown){
								settings.textFeedback.removeClass('gone');
								settings.textFeedback.removeClass('invisible');
								settings.textFeedback.html('Error al enviar los datos.');
		...
		new Swiper('.carousel-swiper-beta-demo .swiper-container',{
			...
			breakpoints:{
				...
				440:{slidesPerView:1,spaceBetween:0}
			}
		});
		
		new Swiper('.testimonials .swiper-container',{
			pagination:'.testimonials .items-pagination', paginationClickable:'.testimonials .items-pagination',
			loop:true, grabCursor:true, centeredSlides:false, autoplay:5000,
			autoplayDisableOnInteraction:false, slidesPerView:1, spaceBetween:55, effect:'slide'});
			...
			if(mainPage.width()){
				mainPage.fullpage({
					...
					afterRender:function(){
						...
						sendMessageForm.initForm({serverUrl:messageServerUrl,});
		...
		$('.scrolldown a, .scroll.down').on('click',function(){
			try{$.fn.fullpage.moveSectionDown();}
			catch(error){$main.animate({
